...
num_photo = len(uos.listdir("/photos"))
...
#sensor.set_gainceiling(5)#4
#sensor.set_auto_gain(False, 22)#10)#22
#sensor.set_brightness()
#sensor.set_auto_whitebal(False, (-6.5, -5.3, -1.1)) #(-5.0, -6., -3.5))#(-6.5, -5.3, -1.1)
#sensor.set_saturation(0)#3
sensor.skip_frames(time = 1700)#500
...
def DefineColor(img, Rect):
    ...
    RGBtoHSV(Elem)
    return Elem
def DefineColorName(ElemX): #Эта функция не используется
...
def main():
    ...
    b_rects = [[0, 120+Plus_V_B, 26, 45], [40, 123+Plus_V_B, 38, 47], [106, 127+Plus_V_B, 43, 50], \
           [181, 123+Plus_V_B, 42, 50], [244, 122+Plus_V_B, 43, 43], [301, 124+Plus_V_B, 19, 44]]
    m_rects = [[110, 51+Plus_V_M, 11, 16], [127, 51+Plus_V_M, 13, 16], [147, 51+Plus_V_M, 13, 16], \
           [166, 51+Plus_V_M, 12, 16], [186, 51+Plus_V_M, 14, 16], [204, 52+Plus_V_M, 14, 16]]
    # [left x, top y, shirina, vysota]
    ...
    img.draw_string(5, 200, MarksCode, (0, 255, 0), 2)
    img.draw_string(5, 218, Code, (0, 255, 0), 2)
    img.save("photos/img-%d.jpg" % num_photo)
